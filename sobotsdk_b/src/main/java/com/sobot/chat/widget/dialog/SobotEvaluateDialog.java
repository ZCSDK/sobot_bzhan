...
public class SobotEvaluateDialog extends SobotActionSheet {
    ...
    private boolean isSolve = true;//是否已解决 默认已解决
    ...
    public SobotEvaluateDialog(Activity context, boolean isFinish, ZhiChiInitModeBase initModel, int current_model, int commentType, String customName, int score, boolean isSolve) {
        super(context);
        this.context = context;
        this.score = score;
        this.isFinish = isFinish;
...
//            });
        ...
        if (isSolve) {
            ...
            sobot_ratingBar_title.setText("非常满意,完美");
            sobot_ratingBar_title.setVisibility(View.VISIBLE);
        } else {
            sobot_btn_no_robot.setChecked(true);
            sobot_btn_ok_robot.setChecked(false);
            sobot_ratingBar_title.setText("非常不满意,特别失望");
            sobot_ratingBar_title.setVisibility(View.VISIBLE);
